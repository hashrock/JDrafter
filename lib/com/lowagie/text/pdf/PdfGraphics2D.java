/*
 * $Id: PdfGraphics2D.java 3427 2008-05-24 18:32:31Z xlv $
 *
...
public class PdfGraphics2D extends Graphics2D {
    ...
    private MediaTracker mediaTracker;    // Added by Jurij Bilas
    ...
    private float jpegQuality = .95f;    // Added by Alexej Suchov
    private float alpha;    // Added by Alexej Suchov
    private Composite composite;    // Added by Alexej Suchov
        ...
        for (Iterator iterator = set.iterator(); iterator.hasNext();) {
            ...
            if (textattribute.equals(TextAttribute.FONT)) {
                ...
                if (iter.getAttributes().get(textattribute) == TextAttribute.UNDERLINE_ON ||
                        iter.getAttributes().get(textattribute)==TextAttribute.UNDERLINE_LOW_ONE_PIXEL) {
                    underline = true;
                }
            } 
...
//            drawGlyphVector(this.font.createGlyphVector(getFontRenderContext(), s), x, y);
            ...
            if (!TextAttribute.WIDTH_REGULAR.equals(fontTextAttributeWidth)) {
                cb.setHorizontalScaling(100.0f / fontTextAttributeWidth.floatValue());            // Check if we need to simulate a bold font.
            ...
            if (!TextAttribute.WIDTH_REGULAR.equals(fontTextAttributeWidth)) {
                cb.setHorizontalScaling(100);            // Restore the original TextRenderingMode if needed.
            }
            ...
            if (strikeThrough){
                ...
                LineMetrics lm=font.getLineMetrics(s,this.getFontRenderContext());
                y-=lm.getAscent()-(lm.getAscent()+lm.getDescent())/2;
                Line2D line=new Line2D.Double(x,y,width+x,y);
                draw(line);
                setStroke(savedStroke);
            }
    ...
    private BaseFont getCachedBaseFont(Font f) {
        synchronized (baseFonts) {
            String fontName = f.getFamily(Locale.ROOT);
            BaseFont bf = (BaseFont) baseFonts.get(fontName);
            if (bf == null) {
                TreeSet familys = new TreeSet(FontFactory.getRegisteredFamilies());
                for (Iterator itr = familys.iterator(); itr.hasNext();) {
                    String s = (String) itr.next();
                    if (fontName.equals(s) || fontName.toLowerCase().equals(s)) {
                        bf = FontFactory.getFont(s, BaseFont.IDENTITY_H, BaseFont.EMBEDDED).getBaseFont();
                        break;
                    }
                }
                if (bf == null) {
                    try {
                        bf = BaseFont.createFont("HeiseiKakuGo-W5", "UniJIS-UCS2-HW-H", BaseFont.NOT_EMBEDDED);
                    } catch (Exception ex) {
